// Re-exported types and prompt builders, copied from daemon for package independence.
...
export function buildPrompt(task: Task, context?: { comments: TaskComment[] }): string {
  ...
  prompt += `## Remote Execution Environment\n`;
  prompt += `Your tools execute on a REMOTE machine, not locally. All file and shell tools (Read, Write, Edit, Glob, Grep, Bash) are provided via the \`remote-tools\` MCP server and run on the target machine's filesystem.\n`;
  prompt += `- There are NO local sandbox restrictions — the remote executor handles path validation.\n`;
  prompt += `- Use absolute paths as provided in the task description.\n`;
  prompt += `- The remote working directory is managed by the executor.\n\n`;
